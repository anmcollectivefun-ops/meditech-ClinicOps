...
export default function StaffPassPage({ params }: { params: StaffPassParams }) {
  ...
  if (loading) {
    return (
      <main className={`min-h-screen flex items-center justify-center p-6 transition-colors duration-500 ${isDarkMode ? 'bg-[#071016] text-white' : 'bg-slate-50 text-slate-900'}`}>
        <div className={`rounded-[32px] border p-8 text-center ${isDarkMode ? 'bg-[#101a22]/75 border-white/10 shadow-sm' : 'bg-white border-slate-200 shadow-sm'}`}>
  ...
  return (
    <main className={`min-h-screen transition-colors duration-500 font-sans pb-20 ${isDarkMode ? 'bg-[#071016] text-white' : 'bg-slate-50 text-slate-900'}`} style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
        ...
        <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-4 px-4 py-3 md:px-6">
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div className="min-w-0">
              ...
              <p className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest mt-0.5 truncate ${isDarkMode ? 'text-cyan-200/60' : 'text-cyan-700/60'}`}>
      ...
      <main className="relative z-10 max-w-[1600px] w-full mx-auto px-4 py-6 md:py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 md:gap-6 items-start">
              ...
              <div className={`flex items-center justify-between ${isNavCollapsed ? 'mb-2' : 'mb-4'}`}>
                {!isNavCollapsed && (
                  <div className="min-w-0 pr-2">
                    <p className={`text-[9px] font-black uppercase tracking-widest ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Panel Obsługi</p>
          ...
          <div className={`${isNavCollapsed ? 'lg:col-span-11' : 'lg:col-span-9'} transition-all duration-300`}>
                    ...
                    <div key={item.label} className={`relative overflow-hidden rounded-[20px] border p-4 shadow-sm transition-colors flex flex-col justify-between min-h-[110px] ${isDarkMode ? 'bg-[#101a22]/75 border-white/10' : 'bg-white border-slate-200'}`}>
                      ...
                      <div className="relative z-10">
                        <div className="flex justify-between items-start">
                          <p className={`text-[9px] font-black uppercase tracking-widest leading-tight ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>{item.label}</p>
            ...
            {activeTab === 'patients' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className={`rounded-[32px] border p-6 shadow-sm ${isDarkMode ? 'bg-[#101a22]/75 border-white/10' : 'bg-white border-slate-200'}`}>
                  <h4 className={`font-black flex items-center gap-2 text-lg mb-4 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                  ...
                  {patients.length === 0 ? <EmptyState isDarkMode={isDarkMode} text="Brak pacjentów dostępnych dla tej roli." /> : (
                    <div className="space-y-3">
                      {patients.slice(0, 30).map((patient: any) => (
                        <div key={patient.id} className={`rounded-2xl border p-4 flex justify-between items-center ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                          <div>
                  ...
                  {appointments.length === 0 ? <EmptyState isDarkMode={isDarkMode} text="Brak wizyt." /> : (
                    <div className="space-y-3">
                      {appointments.slice(0, 30).map((app: any) => {
                        ...
                        return (
                          <div key={app.id} className={`rounded-2xl border p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                            <div>
                              <p className="font-black text-sm">{app.treatment_name || 'Wizyta'}</p>
                              <p className={`text-[10px] font-bold mt-1 ${isDarkMode ? 'text-cyan-400' : 'text-cyan-600'}`}>
                  ...
                  {patientConsents.length === 0 ? <EmptyState isDarkMode={isDarkMode} text="Brak dokumentów." /> : (
                    <div className="space-y-3">
                      {patientConsents.slice(0, 30).map((consent: any) => {
                        ...
                        return (
                          <div key={consent.id} className={`rounded-2xl border p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                            <div>
                              <p className="font-black text-sm">{consent.medical_consent_templates?.title || consent.title || 'Dokument'}</p>
                              <p className={`text-[10px] font-bold mt-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                  ...
                  {portalRequests.length === 0 ? <EmptyState isDarkMode={isDarkMode} text="Brak zgłoszeń pacjentów." /> : (
                    <div className="space-y-3">
                      {portalRequests.slice(0, 30).map((req: any) => {
                        ...
                        return (
                          <div key={req.id} className={`rounded-2xl border p-4 ${isDarkMode ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                            <p className={`text-[10px] font-black uppercase mb-1 ${isDarkMode ? 'text-cyan-400' : 'text-cyan-600'}`}>{req.status}</p>
                            <p className="font-black text-sm">{req.subject || 'Brak tematu'}</p>
                            <p className={`text-xs mt-2 line-clamp-2 ${isDarkMode ? 'text-slate-400' : 'text-slate-600'}`}>{req.message}</p>
                            <p className={`text-[10px] font-bold mt-3 opacity-60 ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>Od: {patient?.first_name} {patient?.last_name}</p>
                          </div>
                        )
                      })}
                    </div>
            ...
            {activeTab === 'qr' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <section className={`rounded-[32px] border p-5 md:p-6 shadow-sm ${isDarkMode ? 'bg-[#101a22]/75 border-white/10' : 'bg-white border-slate-200'}`}>
                  <h4 className={`font-black flex items-center gap-2 text-lg mb-5 ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>
                    <QrCode size={20} className={isDarkMode ? 'text-cyan-200' : 'text-cyan-600'} /> Identyfikacja Pacjenta (Check-in)
                  </h4>

                  <div className={`mb-5 flex flex-wrap gap-2 rounded-2xl border p-2 w-fit ${isDarkMode ? 'bg-white/[0.04] border-white/10' : 'bg-slate-100 border-slate-200'}`}>
                    {[
                  ...
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    <button
                      ...
                      className={`rounded-2xl px-6 py-4 text-sm font-black uppercase flex items-center justify-center gap-2 disabled:opacity-50 transition-all ${isDarkMode ? 'bg-cyan-200 text-[#071016] hover:bg-cyan-300' : 'bg-cyan-600 text-white hover:bg-cyan-700 shadow-md'}`}
                    >
                  ...
                  <div className={`mt-4 rounded-[28px] border p-4 transition-all ${scanMode === 'camera' ? (isDarkMode ? 'ring-1 ring-cyan-200/40' : 'ring-1 ring-cyan-600/40') : ''} ${isDarkMode ? 'bg-black/20 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                    ...
                    {scannerStatus && <p className={`mt-3 rounded-2xl border p-3 text-xs font-bold ${isDarkMode ? 'bg-emerald-900/20 border-emerald-800/50 text-emerald-400' : 'bg-emerald-50 border-emerald-200 text-emerald-700'}`}>{scannerStatus}</p>}
                    {scannerError && <p className={`mt-3 rounded-2xl border p-3 text-xs font-bold ${isDarkMode ? 'bg-amber-900/20 border-amber-800/50 text-amber-500' : 'bg-amber-50 border-amber-200 text-amber-800'}`}>{scannerError}</p>}
                  </div>

                  {error && <p className={`mt-5 rounded-2xl border p-3 text-sm font-bold ${isDarkMode ? 'bg-red-900/20 border-red-800/50 text-red-400' : 'bg-red-50 border-red-200 text-red-700'}`}>{error}</p>}
                  {message && <p className={`mt-5 rounded-2xl border p-3 text-sm font-bold ${isDarkMode ? 'bg-emerald-900/20 border-emerald-800/50 text-emerald-400' : 'bg-emerald-50 border-emerald-200 text-emerald-700'}`}>{message}</p>}
                  {copyMessage && <p className={`mt-5 rounded-2xl border p-3 text-sm font-bold ${isDarkMode ? 'bg-blue-900/20 border-blue-800/50 text-blue-400' : 'bg-blue-50 border-blue-200 text-blue-700'}`}>{copyMessage}</p>}
                </section>
                ...
                {attendeeUnit && (
                  <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4">
                    <div className={`lg:col-span-1 rounded-[32px] border p-5 md:p-6 space-y-4 transition-colors ${isDarkMode ? 'bg-[#101a22]/75 border-white/10' : 'bg-white border-slate-200 shadow-sm'}`}>
                      <div className={`flex items-start justify-between gap-3 border-b pb-4 mb-4 ${isDarkMode ? 'border-white/10' : 'border-slate-100'}`}>
                        <div>
                      ...
                      <div className={`rounded-2xl border p-4 ${isDarkMode ? 'bg-black/20 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                        ...
                        <div className="flex flex-col gap-3">
                          ...
                          <button
                            type="button"
                            onClick={copyQrToken}
                            className={`rounded-xl px-4 py-3 text-[10px] font-black uppercase transition-colors ${isDarkMode ? 'bg-cyan-200 text-[#071016]' : 'bg-cyan-600 text-white hover:bg-cyan-700'}`}
                          >
                            Kopiuj token
                          </button>
                        </div>
                      ...
                      <div className={`rounded-2xl border p-4 text-sm space-y-2 transition-colors ${isDarkMode ? 'bg-black/20 border-white/10' : 'bg-slate-50 border-slate-200'}`}>
                        {canMedicalHistory ? (
                          <>
            ...
            {activeTab === 'analytics' && (
              <div className="space-y-6 animate-in fade-in duration-300">
                <div className={`rounded-[32px] border p-8 shadow-sm text-center transition-colors ${isDarkMode ? 'bg-[#101a22]/75 border-white/10' : 'bg-white border-slate-200'}`}>
                  <Stethoscope size={48} className={`mx-auto mb-4 ${isDarkMode ? 'text-cyan-200' : 'text-cyan-600'}`} />
...
const StaffPanel = ({ icon, title, children, isDarkMode }: any) => (
  <section className={`rounded-[32px] border p-5 md:p-6 transition-colors ${isDarkMode ? 'bg-[#101a22]/75 border-white/10' : 'bg-white border-slate-200 shadow-sm'}`}>
    <div className="mb-5 flex items-center justify-between gap-4">
      ...
      <span className={`rounded-xl border px-3 py-1.5 text-[9px] font-black uppercase tracking-widest ${isDarkMode ? 'bg-white/[0.04] border-white/10 text-white/45' : 'bg-slate-50 border-slate-200 text-slate-500'}`}>
        ClinicOps
      </span>
    </div>
...
const ActionButton = ({ label, icon, disabled, onClick, isDarkMode }: any) => (
  <button
    ...
    disabled={disabled}
    onClick={onClick}
    className={`w-full rounded-2xl px-4 py-4 text-sm font-black uppercase flex items-center justify-center gap-2 disabled:opacity-45 disabled:cursor-not-allowed transition-colors ${isDarkMode ? 'bg-cyan-200 text-[#071016]' : 'bg-cyan-600 text-white hover:bg-cyan-700'}`}
  >
    {icon}
    {label}
  </button>
...
const SmallButton = ({ children, disabled, onClick, isDarkMode }: any) => (
  <button
    ...
    disabled={disabled}
    onClick={onClick}
    className={`rounded-xl px-4 py-2 text-[10px] font-black uppercase transition-colors disabled:opacity-45 disabled:cursor-not-allowed ${isDarkMode ? 'bg-cyan-200 text-[#071016]' : 'bg-cyan-600 text-white hover:bg-cyan-700'}`}
  >
    {children}
  </button>
)
...
const ItemRow = ({ title, subtitle, status, action, isDarkMode }: any) => {
  ...
  const statusClass = normalizedStatus.includes('wydano') || normalizedStatus.includes('odrzucony')
    ? 'text-red-500'
