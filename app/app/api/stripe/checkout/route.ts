...
export async function POST(req: Request) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${process.env.NEXT_PUBLIC_SITE_URL}/dashboard/events/${eventId}?payment=success`,
      cancel_url: `${process.env.NEXT_PUBLIC_SITE_URL}/dashboard/events/${eventId}?payment=canceled`,
      metadata: {
        eventId: eventId, 
      },
    });
